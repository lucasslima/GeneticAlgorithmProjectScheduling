...
Instance::Instance(string filename) { load(filename); }
void Instance::load(std::string fileName) {
  ...
  if (readSucess) {
    ...
    for (Json::ValueIterator rIterator = availableResources.begin();
         rIterator != availableResources.end(); rIterator++)
      mRenewableResourceAvailability.push_back(atoi((*rIterator).asCString()));
    this->project = new Project(numberOfJobs.asInt(), horizon.asInt(),
                                resourceNumber.asInt());
    for (Json::ValueIterator jIterator = instance["jobs"].begin();
      ...
      for (Json::ValueIterator rIterator = request.begin();
           rIterator != request.end(); rIterator++) {
        project->jobDuration[j] = atoi((*rIterator)["duration"].asCString());
        Json::Value resourceRequest = (*rIterator)["renews"];
        int rpos = 0;
        for (Json::ValueIterator r = resourceRequest.begin();
             r != resourceRequest.end(); r++)
